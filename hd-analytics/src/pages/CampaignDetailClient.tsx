...
import { completeAction, issueAction } from '@/lib/dashboard/campaigns/mock-actions'
...
export default function CampaignDetailClient({
  ...
  // No server to re-render against in a static bundle — reload the page so it
  // re-reads the (localStorage-backed) mock-actions status override.
  const onIssue = () => {
    ...
    <div className="min-h-screen" style={{ backgroundColor: colors.bg }}>
      <header className="sticky top-0 z-30 backdrop-blur-xl border-b border-dash-card-border" style={{ backgroundColor: colors.headerBg }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-3 pb-2 flex items-center justify-between">
          <div>
            <span className="eyebrow text-hd-gold">Campaign · {campaign.segment_key}</span>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        {incrementality && (
          <section style={{ backgroundColor: colors.card, border: `1px solid ${colors.cardBorder}` }} className="p-5">
            <div className="flex items-baseline justify-between mb-4">
              <h3 className="font-display text-[1.1rem] text-dash-text">Treatment vs holdout</h3>
        ...
        {compareData.length > 0 && (
          <section style={{ backgroundColor: colors.card, border: `1px solid ${colors.cardBorder}` }} className="p-5">
            <h3 className="font-display text-[1.1rem] text-dash-text mb-4">Projected vs actual</h3>
        ...
        {/* Lessons */}
        <section style={{ backgroundColor: colors.card, border: `1px solid ${colors.cardBorder}` }} className="p-5">
          <h3 className="font-display text-[1.1rem] text-dash-text mb-3">Lessons learned</h3>
        ...
        <section style={{ backgroundColor: colors.card, border: `1px solid ${colors.cardBorder}` }} className="p-5">
          <div className="flex items-baseline justify-between mb-3">
            <h3 className="font-display text-[1.1rem] text-dash-text">Targets</h3>
            <div className="flex items-center gap-2">
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {filteredTargets.map((t) => (
                  <tr key={`${t.campaign_id}-${t.user_id}`} style={{ borderBottom: `1px solid ${colors.divider}` }}>
