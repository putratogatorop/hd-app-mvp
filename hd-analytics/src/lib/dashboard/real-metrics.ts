...
import { REFERRALS, VOUCHERS } from '@/fixtures/orders.fixture'
...
export async function getOverviewMetrics(
  ...
  const rows = await fetchEnrichedOrders(filters)
  const allRows = await fetchEnrichedOrders({ excludeCancelled: true })
...
export async function getGiftMetrics(
  ...
  const rows = await fetchEnrichedOrders(giftFilters)

  // "Total" gifts = all-time, independent of current filter.
  const allGifts = await fetchEnrichedOrders({ isGift: true, excludeCancelled: true })
...
export async function getTransactionalMetrics(
  ...
  const [rows, items] = await Promise.all([
    fetchEnrichedOrders(txFilters),
    fetchEnrichedOrderItems(txFilters),
  ])
...
const voucherHintById = new Map(VOUCHERS.map((v) => [v.id, v.redemptionRateHint]))
...
export async function getOverviewRealData(
  ...
  const [curr, prev, items] = await Promise.all([
    fetchEnrichedOrders(currFilters),
    fetchEnrichedOrders(prevFilters),
    fetchEnrichedOrderItems(currFilters),
  ])
  ...
  // Referrals — static fixture array standing in for the live `referrals` table.
  const refCurr = REFERRALS.filter((r) => r.created_at >= (currFilters.from ?? startOfDaysAgo(30))).length
  const refPrev = REFERRALS.filter(
    (r) =>
      r.created_at >= (prevFilters.from ?? startOfDaysAgo(60)) &&
      r.created_at < (prevFilters.to ?? startOfDaysAgo(30)),
  ).length
  ...
  const kpi: KPIData = {
    ...
    referralConversions: mk(refCurr, refPrev, dayKeys.map(() => 0)),
  }
  ...
  const vouchers: VoucherPerformance[] = Array.from(voucherAgg, ([id, row]) => {
    const hint = voucherHintById.get(id) ?? 0.6
    const issued = Math.max(row.redeemed, Math.round(row.redeemed / hint))
    return {
      id,
      code: row.code,
      title: row.title,
      ...
      redemptionRate: issued > 0 ? Math.round((row.redeemed / issued) * 1000) / 10 : 0,
      roi: row.discount > 0 ? Math.round((row.revenue / row.discount) * 10) / 10 : 0,
    }
  }).sort((a, b) => b.redeemed - a.redeemed)

  // ── Referral funnel ─────────────────────────────────
  const refs = REFERRALS.filter((r) => r.created_at >= (currFilters.from ?? startOfDaysAgo(30)))
...
export async function getRFMData(): Promise<RFMData> {
